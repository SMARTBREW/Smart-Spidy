...
export const TrainingDataTable: React.FC<TrainingDataTableProps> = ({ stats }) => {
  ...
  const [sourceFilter, setSourceFilter] = useState<'all' | 'chat' | 'auto' | 'imported' | 'manual'>('all');
  ...
  const filteredData = trainingData.filter(item => {
    ...
    const matchesQuality = qualityFilter === 'all' || 
                          (qualityFilter === 'high' && item.qualityScore >= 0.8) ||
                          (qualityFilter === 'medium' && item.qualityScore >= 0.5 && item.qualityScore < 0.8) ||
                          (qualityFilter === 'low' && item.qualityScore < 0.5);
    return matchesSearch && matchesSource && matchesQuality;
  });

  const localStats = {
    total: trainingData.length,
    highQuality: trainingData.filter(d => d.qualityScore >= 0.8).length,
    autoGenerated: trainingData.filter(d => d.source === 'auto').length,
    avgQuality: trainingData.length > 0 ? (trainingData.reduce((sum, d) => sum + d.qualityScore, 0) / trainingData.length).toFixed(2) : '0'
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatCard
          ...
          value={stats?.trainingDataCount || 0}
          icon={Database}
        ...
        <StatCard
          ...
          value={localStats.autoGenerated}
          icon={RefreshCw}
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="text-left py-4 px-6 font-semibold text-gray-900">Question & Answer</th>
                <th className="text-left py-4 px-6 font-semibold text-gray-900">Source</th>
                <th className="text-left py-4 px-6 font-semibold text-gray-900">Quality</th>
                  ...
                  >
                    <td className="py-5 px-6">
                      <div className="max-w-md space-y-2">
                        ...
                        <div>
                          <p className="font-medium text-gray-900 text-sm mb-1">Answer:</p>
                    ...
                    <td className="py-5 px-6">
                      <div className="flex items-center justify-end space-x-2">
                        <button
                          ...
                          title="View training data details"
                        >
